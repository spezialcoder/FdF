...
/*   Updated: 2024/06/04 18:08:21 by lsorg            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void apply_rotation_matrix(t_engine_config *config, t_unit_vectors vectors);
...
void fdf_show_space(t_engine_config *config) {
    ...
    t_position origin;
    t_axis_color colors;
    int xOffset = config->image->width-100;
    int yOffset = 80;

    vectors = get_unit_vectors(50);
    colors = get_axis_colors(config);
    apply_rotation_matrix(config,vectors);
    origin = (t_position){.x=xOffset,.y=yOffset};
    img_draw_line(config->image,origin,(t_position){
        .x = vectors.xaxis.matrix[0][0] + xOffset,
        .y = vectors.xaxis.matrix[1][0] + yOffset
        },colors.x_color);
    img_draw_line(config->image,origin,(t_position){
            .x = vectors.yaxis.matrix[0][0] + xOffset,
            .y = vectors.yaxis.matrix[1][0] + yOffset
    },colors.y_color);

    img_draw_line(config->image,origin,(t_position){
            .x = vectors.zaxis.matrix[0][0] + xOffset,
            .y = vectors.zaxis.matrix[1][0] + yOffset
    },colors.z_color);
